...
def handle_download():
    ...
    if not is_success:
        return jsonify({"error": "Failed to retrieve folder contents"}), 500

    # 获取目录结构
    directory_structure = _get_directory_structure(gdrive_file, previous_path="")

    # 设置输出目录
    output_dir = data.get("output", os.getcwd())
    ...
    # 返回 ZIP 文件路径
    return jsonify({"zip_file": f"{output_dir}/downloaded_files_*.zip"}), 200
